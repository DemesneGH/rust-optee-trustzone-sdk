...
TAR_TOP_DIR_NAME="incubator-teaclave-trustzone-sdk-${RELEASE_VERSION}"
...
case "$1" in
    prepare)
        ...
        wget "https://github.com/apache/incubator-teaclave-trustzone-sdk/archive/refs/tags/${TAG}.tar.gz"
        tar xzvf "${TAG}.tar.gz"

        mv "incubator-teaclave-trustzone-sdk-${RELEASE_VERSION}-rc.${RC_NUMBER}" "${TAR_TOP_DIR_NAME}"
    ...
    upload)
        ...
        svn add "${SVN_RC_DIR}"
        svn ci --username "${ASF_USERNAME}" --password "${ASF_PASSWORD}" -m "Add TrustZone SDK ${SVN_RC_DIR}"
    ...
    finalize)
        ...
        svn ci --username "${ASF_USERNAME}" --password "${ASF_PASSWORD}" -m "Add TrustZone SDK ${RELEASE_VERSION} final release"
