...
fn open_session(_params: &mut Parameters, _sess_ctx: &mut WalletSession) -> optee_utee::Result<()> {
    ...
    // The WalletSession::default() method is called to create a new instance
    // before open_session(), so no need to init the WalletSession here.
    // Related code: optee-utee/macros/src/lib.rs
    Ok(())
}
...
fn remove_wallet(
    ...
    db_client.delete_entry::<Wallet>(&input.wallet_id.to_string())?;
...
fn derive_address(
    ...
    let wallet = db_client
        .get::<Wallet>(&input.wallet_id.to_string())
