...
// For Storable<K>, Any K type can be used as unique id as long as
// it implements TryFrom<String> + Into<String> + Clone
impl Storable<String> for ExampleData {
...
pub fn test() -> anyhow::Result<()> {
    ...
    let loaded_example_data = db_client.get::<_, ExampleData>(&example_data.id)?;
    anyhow::ensure!(
        loaded_example_data == example_data,
        "Loaded example_data is not equal to the original example_data"
    );
    // List all entries in db
    let entries = db_client.list_entries::<_, ExampleData>()?;
    trace_println!("Entries: {:?}", entries);
    // Delete entry from db
    db_client.delete_entry::<_, ExampleData>(&example_data.id)?;
